...
public class ProtoTickList<T> implements TickList<T> {
   ...
   public void scheduleTick(BlockPos var1, T var2, int var3, TickPriority var4) {
      int var5 = this.levelHeightAccessor.getSectionIndex(var1.getY());
      if (var5 >= 0 && var5 < this.levelHeightAccessor.getSectionsCount()) {
         ChunkAccess.getOrCreateOffsetList(this.toBeTicked, var5).add(ProtoChunk.packOffsetCoordinates(var1));
      }
   }
